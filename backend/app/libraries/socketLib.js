        ...
        socket.on('set-user', (authToken) => {
            tokenLib.verifyClaimWithoutSecret(authToken, (err, userData) => {
                if (err) {
                        ...
                        if (err) {
                            ...
                            userModel.findOne({userName:issueDetails.reporterName},(err,userDetails)=>{
                                if (err) {
                                    ...
                                    console.log('emitting to ',issueDetails.reporterName);
                                    
                                    socket.emit(reporterId, info)
                            // notifier.saveNotifications(reporterId,info.msg)
                    ...
                    // console.log('emitting to ************',userData.data.userName);
                    
                    // socket.emit(currentUser, info) 
                        // socket.emit(info.assigneeId, info.msg)
                            // notifier.saveNotifications(info.assigneeId,info.msg)
                            ...
                            redisLib.showHash(info.issueId + '{Watcher-list}', (err, data) => {
                                if (err) {
                                    ...
                                    watcherList.forEach(watcher => {
                                        ...
                                        // notifier.saveNotifications(watcher,info.msg)
                                    });
                                }
                            })
